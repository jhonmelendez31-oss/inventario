...
function agregarVehiculo() {
  ...
  const placa =
    normalizar(document.getElementById('nuevaPlaca').value);
  ...
  const danos =
    document.getElementById('nuevosDanos').value.trim();
  ...
  mostrarToast(`✅ Vehículo ${placa} guardado`);
}
...
function editarVehiculo(placa) {
  ...
  if (!vehiculo) {
    ...
    mostrarToast('⚠️ Vehículo no encontrado');
  ...
  mostrarToast(`✏️ Vehículo ${placa} actualizado`);
...
function eliminarVehiculo(placa) {
  ...
  const confirmar = confirm(`¿Eliminar vehículo ${placa}?`);
  ...
  mostrarToast(`🗑️ Vehículo ${placa} eliminado`);
  ...
  placas.forEach(placa => {
    ...
    if (v) {
      ...
      const dot =
        ...
        style="background:${coloresCSS[v.color] || '#999'}"></span>`;
...
function actualizarBusquedaRapida() {
  ...
  const input =
    document.getElementById('busquedaRapida');
...
function iniciarSistema() {
  ...
  document
    ...
    .addEventListener('input', function(e) {
      ...
      const resultado =
        document.getElementById('resultadoRapido');
